...
function build_calendar($month, $year) {
    ...
    $doc_id = $_SESSION['unique_id'];
    $get_doc = "select * from users where unique_id = '$doc_id' ";
    ...
    $calendar.= "<a class='btn btn-xs btn-primary' href='?update_doc_appointment=" . $id . "&month=" . date('m', mktime(0, 0, 0, $month - 1, 1, $year)) . "&year=" . date('Y', mktime(0, 0, 0, $month - 1, 1, $year)) . "'>Previous Month</a> ";

    $calendar.= " <a class='btn btn-xs btn-primary' href='?update_doc_appointment=" . $id . "&month=" . date('m') . "&year=" . date('Y') . "'>Current Month</a> ";

    $calendar.= "<a class='btn btn-xs btn-primary' href='?update_doc_appointment=" . $id . "&month=" . date('m', mktime(0, 0, 0, $month + 1, 1, $year)) . "&year=" . date('Y', mktime(0, 0, 0, $month + 1, 1, $year)) . "'>Next Month</a></center><br>";
    ...
    while ($currentDay <= $numberDays) {
        ...
        if ($date < date('Y-m-d')) {
            ...
            $calendar.="<td class='$today'><h4>$currentDay</h4> <a href='index.php?make_leave=" . $date . "&doc_booking=" . $id . "' class='btn btn-success btn-xs'>Make Leave</a>";
        }
